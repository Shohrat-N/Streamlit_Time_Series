...
tabs=["Dataset:Population of Azerbaijan", "Data Preparation for Time Series", "Model", "Forecast", "Visualisation"]


page=st.sidebar.radio("Contents",tabs)
...
data=pd.read_excel("Data/Population_76.xlsx") 
...
if page=="Dataset:Population of Azerbaijan":
    with header:
...
if page=="Data Preparation for Time Series":
...
if page=="Model":
    ...
    image = Image.open("Data/Untitled.png")
...
train=data[:60]
test=data[60:]
model_arima=ARIMA(train, order=(2, 1, 0), trend="t")
...
train.index = pd.date_range(start='1950', end='2010', freq='A')

if page=="Model":
    fig, ax = plt.subplots()
    ax = train.loc['1950':].plot(ax=ax)
    plt.show()
    with model:
        st.subheader("Result of Train dataset")
        st.pyplot(plot_predict(model_arima_fit, '1950', '2010', ax=ax))
...
model2=ARIMA(data['Population'],order=(2,1,0), trend='t')
model2=model2.fit()
...
forc=pd.DataFrame(forc)
forc.reset_index(inplace=True)

t=forc.Forecasting.iloc[0]
...
if page == "Forecast":
    with forecastt:
        ...
        fh_selection=st.selectbox("Choose year",["2023","2024","2025","2026","2027", "2028", "2029", "2030"])
        button=st.button("Forecast")
        if button==True:
                if fh_selection == '2023':
                	st.write(forc.Forecasting.iloc[0])
                elif fh_selection == '2024':
                	st.write(forc.Forecasting.iloc[1])
                elif fh_selection == '2025':
                	st.write(forc.Forecasting.iloc[2])
                elif fh_selection == '2026':
                	st.write(forc.Forecasting.iloc[3])
                elif fh_selection == '2027':
                	st.write(forc.Forecasting.iloc[4])        
                elif fh_selection == '2028':
                	st.write(forc.Forecasting.iloc[5])
                elif fh_selection == '2029':
                 	st.write(forc.Forecasting.iloc[6] )
                elif fh_selection == '2030':
                 	st.write(forc.Forecasting.iloc[7])
...
if page=="Visualisation":
    ...
    with forecastt:
        st.subheader("Visualisation of 2023-2030")    
        st.pyplot(fig)
        #st.write(forc)
